...
async def login(
    ...
    if database_url and username != "admin":
        try:
            from sardis.services.auth_service import AuthService
...
async def bootstrap_api_key(
    ...
    from sardis.middleware.auth import get_api_key_manager
...
async def signup(request: Request, body: SignupRequest):
    ...
    try:
        from sardis.email_templates import send_welcome_email
...
async def register_user(request: Request, body: RegisterRequest):
    ...
    from sardis.analytics.posthog_tracker import SIGNUP_COMPLETED, identify_user, track_event
    ...
    try:
        from sardis.email_templates import send_email
...
async def forgot_password(request: Request, body: ForgotPasswordRequest):
    ...
    if token:
        from sardis.email_templates import send_password_reset_email
...
async def delete_account(request: Request, user: UserInfo = Depends(require_auth)):
    ...
    from sardis.analytics.posthog_tracker import track_event
...
async def _resolve_principal(
    ...
    from sardis.middleware.auth import get_api_key
