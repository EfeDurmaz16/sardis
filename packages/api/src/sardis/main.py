...
def _bootstrap_monorepo_sys_path() -> None:
    ...
    This helps local dev commands like:
      `uv run uvicorn sardis.main:create_app --factory`
...
def create_app(settings: SardisSettings | None = None) -> FastAPI:
    ...
    if os.getenv("SARDIS_ENABLE_CARDS", "").lower() in ("1", "true", "yes"):
        from sardis.repositories.card_repository import CardRepository
    ...
    # Spending mandates
    try:
        from sardis.routes.authority import spending_mandates as spending_mandates_router
        app.include_router(
    ...
    try:
        from sardis.routes.identity import agent_auth as agent_auth_router
    ...
    try:
        from sardis.routes.providers import polar_webhook as polar_webhook_router
        app.include_router(
    ...
    try:
        from sardis.middleware.sso import router as sso_router
