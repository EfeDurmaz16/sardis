...
async def login(
    ...
    if database_url and username != "admin":
        try:
            from sardis_server.services.auth_service import AuthService
...
async def bootstrap_api_key(
    ...
    from sardis_server.middleware.auth import get_api_key_manager
...
async def signup(request: Request, body: SignupRequest):
    ...
    try:
        from sardis_server.email_templates import send_welcome_email
    ...
    return SignupResponse(
        ...
        next_steps=[
            ...
            "POST /api/v2/sandbox/policy-check — Test a policy check",
        ],
    )
...
async def register_user(request: Request, body: RegisterRequest):
    ...
    from sardis_server.analytics.posthog_tracker import SIGNUP_COMPLETED, identify_user, track_event
    ...
    try:
        from sardis_server.email_templates import send_email
...
async def forgot_password(request: Request, body: ForgotPasswordRequest):
    ...
    if token:
        from sardis_server.email_templates import send_password_reset_email
...
async def delete_account(request: Request, user: UserInfo = Depends(require_auth)):
    ...
    from sardis_server.analytics.posthog_tracker import track_event
...
async def _resolve_principal(
    ...
    from sardis_server.middleware.auth import get_api_key
