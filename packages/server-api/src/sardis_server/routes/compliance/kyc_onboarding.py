...
logger = logging.getLogger("sardis_server.api.kyc_onboarding")
...
async def handle_kyc_webhook(request: Request) -> dict:
    ...
    if mapped_status in ("approved", "declined"):
        try:
            ...
            if user_email:
                from sardis_server.email_templates import send_kyc_status_email
...
async def _on_kyc_approved(user_id: str, organization_id: str | None) -> None:
    ...
    if organization_id:
        try:
            from sardis_server.dependencies import get_container
            ...
            try:
                ...
                if user_email:
                    from sardis_server.email_templates import send_api_key_generated_email
