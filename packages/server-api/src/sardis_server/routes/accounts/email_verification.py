...
from sardis_server.routes.accounts.auth import UserInfo, require_auth
...
async def _send_verification_email(to_email: str, verification_link: str) -> None:
    """Send a verification email via Resend (preferred) or SMTP fallback.

    Uses the centralized ``send_email`` from ``email_templates``.
    No-op if neither RESEND_API_KEY nor SMTP_HOST is configured.
    """
    ...
    sent = await send_email(
        to=to_email,
        subject="Verify your Sardis email address",
        html_body=html_body,
    )
    if sent:
        _logger.info("Verification email sent to %s", to_email)
    else:
        _logger.info("No email transport configured — skipping verification email to %s (link: %s)", to_email, verification_link)
...
async def send_verification_email(
    ...
    await _send_verification_email(to_email=to_email, verification_link=verification_link)
