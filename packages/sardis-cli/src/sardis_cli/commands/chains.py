...
def route(ctx, from_chain: str, to_chain: str, amount: float, token: str):
    ...
    try:
        ...
        if result.get("bridges"):
            ...
            console.print("  - [cyan]Across Protocol[/cyan] (fastest, ~2 min)")
            console.print("  - [cyan]Stargate Finance[/cyan] (LayerZero, ~5 min)")
            console.print("  - [cyan]Circle CCTP[/cyan] (native USDC, ~15 min)")

    except APIError as e:
        if "404" in str(e) or "not found" in str(e).lower():
            console.print("\n[yellow]Route endpoint not available on this server.[/yellow]")
            console.print(f"\nManual cross-chain options for {token}:")
            console.print("  - [cyan]Across Protocol[/cyan] — https://across.to")
            console.print("  - [cyan]Stargate Finance[/cyan] — https://stargate.finance")
            console.print("  - [cyan]Circle CCTP[/cyan] — https://www.circle.com/en/cross-chain-transfer-protocol")
        else:
            console.print(f"[red]Error: {e.message}[/red]")
    finally:
        client.close()
