...
class EnhancedWalletManager:
    ...
    async def rotate_wallet_key(
        ...
        NOT IMPLEMENTED: Key rotation requires integration with Turnkey's
        key rotation API. The previous implementation only tracked rotation
        events in memory without actually rotating keys at the MPC provider
        level, giving false confidence that rotation occurred.

        For manual key rotation, use the Turnkey dashboard directly.
        """
        raise NotImplementedError(
            "Key rotation requires Turnkey API integration. "
            "Manual rotation available via Turnkey dashboard."
        )
    ...
    async def emergency_key_rotation(
        ...
        NOT IMPLEMENTED: See rotate_wallet_key for rationale.
        For emergencies, freeze the wallet and rotate keys via the
        Turnkey dashboard directly.
        """
        raise NotImplementedError(
            "Emergency key rotation requires Turnkey API integration. "
            "For emergencies: freeze the wallet immediately, then rotate "
            "keys via the Turnkey dashboard."
        )
