...
class TestTierLimits:
    ...
    def test_dev_tier_limits(self):
        from sardis_server.services.usage_metering import TIER_LIMITS
        dev = TIER_LIMITS["dev"]
...
class TestCheckLimit:
    ...
    async def test_under_limit_returns_true(self):
        ...
        from sardis_server.services.usage_metering import UsageMeteringService, UsageSummary
...
class TestSDKMetrics:
    ...
    def test_pypi_packages_defined(self):
        from sardis_server.services.sdk_metrics import PYPI_PACKAGES
        assert "sardis" in PYPI_PACKAGES
        assert len(PYPI_PACKAGES) > 0

    def test_npm_packages_defined(self):
        from sardis_server.services.sdk_metrics import NPM_PACKAGES
        assert "@sardis/sdk" in NPM_PACKAGES

    def test_sdk_metrics_router_routes(self):
        from sardis_server.routes.developer.sdk_metrics import router
