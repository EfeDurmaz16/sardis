...
class FakeComplianceResult:
    ...
    provider: str = "rules"
    rule_id: str = "baseline"


@dataclass
class FakeChainReceipt:
    tx_hash: str = "0xdeadbeef"
    chain: str = "base"
    audit_anchor: str = "anchor_001"
    status: str = "submitted"
    block_number: int | None = 12345
...
def _build_orchestrator(
    *,
    ...
    chain_fail: bool = False,
    ledger_fail: bool = False,
    spend_record_fail: bool = False,
    with_kya: bool = False,
    kya_allowed: bool = True,
    kya_reason: str = "OK",
    kya_level: str = "verified",
) -> PaymentOrchestrator:
    """Build a PaymentOrchestrator with mock dependencies."""
    ...
    # InMemoryReconciliationQueue and InMemoryDedupStore require dev environment
...
async def test_kya_fail_closed_blocks_payment():
    ...
    Note: KYA check requires agent_id on the payment object. PaymentMandate
    (slots=True) doesn't have agent_id, so we use a mock chain whose payment
    has agent_id set.
    """
    orch = _build_orchestrator(
        with_kya=True,
    ...
    chain = _make_chain()
    # Wrap payment in a MagicMock that has agent_id
    mock_payment = MagicMock(wraps=chain.payment)
    mock_payment.agent_id = "test_agent"
    mock_payment.mandate_id = chain.payment.mandate_id
    mock_payment.chain = chain.payment.chain
    mock_payment.token = chain.payment.token
    mock_payment.amount_minor = chain.payment.amount_minor
    mock_payment.destination = chain.payment.destination
    mock_payment.subject = chain.payment.subject

    mock_chain = MagicMock(wraps=chain)
    mock_chain.payment = mock_payment

    with pytest.raises(KYAViolationError, match="agent_suspended"):
        await orch.execute_chain(mock_chain)
...
async def test_audit_log_tracks_phases():
    ...
    # chain_execution may be tracked under policy_state_update in settlement flow
