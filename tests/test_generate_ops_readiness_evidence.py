...
def test_ops_evidence_generator_writes_artifact(tmp_path: Path) -> None:
    ...
    assert payload["alert_routing"]["routing_checks"]["critical_includes_pagerduty"] is True
    assert payload["alert_routing"]["routing_checks"]["critical_has_human_route"] is True
    assert payload["alert_routing"]["routing_checks"]["pagerduty_cooldown_configured"] is True
...
def test_ops_evidence_generator_strict_routing_fails_without_pagerduty(tmp_path: Path) -> None:
    drill = {
        "drill_id": "drill_3",
        "incident_started_at": "2026-02-26T09:00:00Z",
        "failover_mode_activated_at": "2026-02-26T09:10:00Z",
    ...
    output_path = tmp_path / "ops-readiness.json"
    env = dict(os.environ)
    env["SARDIS_ALERT_SEVERITY_CHANNELS_JSON"] = json.dumps({"critical": ["websocket"]})
    env["SARDIS_ALERT_CHANNEL_COOLDOWNS_JSON"] = json.dumps({"slack": 60})
    env.pop("PAGERDUTY_ROUTING_KEY", None)
    ...
    assert result.returncode != 0
    assert "strict routing checks failed" in (result.stdout + result.stderr)
