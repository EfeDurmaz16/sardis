...
class TestCreateStripeOnrampSession:
    ...
    async def test_success(self):
        from sardis_api.routers.onramp import _create_stripe_onramp_session
        ...
        with (
            ...
            patch("sardis_api.routers.onramp.httpx.AsyncClient", return_value=mock_client),
        ):
...
class TestStripeOnrampLinkUrl:
    ...
    async def test_link_url_contains_expected_params(self):
        ...
        from sardis_api.routers.onramp import get_stripe_onramp_link

        # Mock the wallet resolution and auth
        wallet_addr = "0xAABBCCDDEEFF00112233445566778899AABBCCDD"
        mock_principal = MagicMock()

        with patch("sardis_api.routers.onramp._resolve_wallet_address", return_value=wallet_addr):
