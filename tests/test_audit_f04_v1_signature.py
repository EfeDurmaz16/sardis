...
def test_v2_signature_accepted():
    ...
    # Sign V2-style payload (WITH merchant_domain + AP2 visibility fields + binding fields) - correct format
    canonical = "|".join([
        mandate_id, agent_id, "1000000", "USDC", "base",
        destination, merchant_domain, audit_hash, "1", "human_present",
        "",  # cart_mandate_hash (empty default)
        "",  # approval_context_hash (empty default)
    ]).encode()
