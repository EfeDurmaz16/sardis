...
```text
packages/reference-api/sardis_server/routes/<domain>/<module>.py
```
...
Keep `src/sardis` separate from `packages/reference-api/sardis_server`. The former is the public client facade; the latter is the server implementation.
