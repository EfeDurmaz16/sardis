...
| --- | --- | --- | --- |
| `52 MCP tools` | `README.md:261`, `landing/src/App.jsx:610` | `bash scripts/release/readiness_check.sh` | `MCP registry parity: 52 definitions = 52 handlers` |
| `150+ tests` | `README.md:257` | `pytest --collect-only -q` | `collected 845 items` (`783 selected`) |
...
| `Env docs match runtime usage` | `.env.example` | `bash scripts/release/env_doc_check.sh` | `documented all runtime env vars (runtime=75, documented=98)` |
...
| `Alembic revisions align with SQL migration set` | `packages/sardis-api/alembic/versions`, `packages/sardis-api/migrations` | `bash scripts/release/migration_alignment_check.sh` | contiguous revision chain with head aligned at `014` |
...
[env-doc][pass] documented all runtime env vars (runtime=75, documented=98)
...
[migrations][pass] aligned heads at revision 014 with contiguous Alembic chain
...
[readiness][pass] pytest collected items: 845 (selected: 783)
