...
| `150+ tests` | `README.md:279` | `pytest --collect-only -q` | `collected 854 items` (`792 selected`) |
...
| `5 protocols (AP2, TAP, UCP, A2A, x402)` | `README.md:266` | file presence + exports | AP2 (`packages/sardis-protocol/src/sardis_protocol/schemas.py:12`), TAP (`packages/sardis-protocol/src/sardis_protocol/tap.py:23`), UCP (`packages/sardis-ucp/src/sardis_ucp/__init__.py:51`), A2A (`packages/sardis-a2a/src/sardis_a2a/__init__.py:1`), x402 (`packages/sardis-protocol/src/sardis_protocol/x402.py:1`) |
| `Env docs match runtime usage` | `.env.example` | `bash scripts/release/env_doc_check.sh` | `documented all runtime env vars (runtime=86, documented=110)` |
...
| `Alembic revisions align with SQL migration set` | `packages/sardis-api/alembic/versions`, `packages/sardis-api/migrations` | `bash scripts/release/migration_alignment_check.sh` | contiguous revision chain with head aligned at `018` |
| `Deployment configs are internally consistent` | `vercel.json`, `landing/vercel.json`, `.github/workflows/deploy.yml`, `.github/workflows/ci.yml` | `bash scripts/release/deployment_config_check.sh` | validates landing/API deploy jobs, build dirs, env targets, and migration execution |
...
| `Smart contract tests pass post-remediation` | `contracts/test/*.t.sol` | `FOUNDRY_OFFLINE=true forge test --root contracts` | `102 passed, 0 failed, 0 skipped` |
| `Launch copy uses verified metrics` | `docs/marketing/product-hunt-launch.md`, `docs/marketing/social-launch-kit.md` | manual review against this file | hard metrics (19 packages, 57 tools, 5 chains/protocols) now explicitly tied to `claims-evidence.md` |
...
[env-doc][pass] documented all runtime env vars (runtime=86, documented=110)
...
[readiness] validating package metadata and licenses
[metadata][pass] pyproject metadata is complete
[metadata][pass] package.json metadata is complete
[readiness] validating migration alignment
[migrations][pass] aligned heads at revision 018 with contiguous Alembic chain
[readiness] validating deployment config consistency
[deploy] deployment configuration checks passed
[readiness][pass] MCP registry parity: 57 definitions = 57 handlers
[readiness][pass] pytest collected items: 854 (selected: 792)
