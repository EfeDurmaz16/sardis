...
- FastAPI backend in `packages/api/src/sardis_api`, with a large `main.py` composition root, many routers, middleware, repositories, services, and lifecycle jobs.
...
- Small internal composition modules inside `packages/api/src/sardis_api` may be extracted from `main.py`.
