...
- FastAPI backend in `packages/server-api/src/sardis_server`, with a large `main.py` composition root, many routers, middleware, repositories, services, and lifecycle jobs.
...
- Small internal composition modules inside `packages/server-api/src/sardis_server` may be extracted from `main.py`.
