...
| `routers/enterprise_support.py`, `routers/sdk_metrics.py` implementations | `routes/developer/*` with compatibility module aliases in `routers/` | Moves contributor/developer support ticketing and public SDK install metrics out of the flat router bucket while preserving existing HTTP paths. |
...
11. Developer and contributor-facing tools: completed for
   `webhook_subscriptions`, `enterprise_support`, and `sdk_metrics`.
