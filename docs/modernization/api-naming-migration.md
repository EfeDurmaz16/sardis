...
| `routers/analytics.py`, `routers/alerts.py`, `routers/ws_alerts.py`, `routers/event_stream.py`, `routers/reports.py`, `routers/reliability.py`, `routers/dashboard_metrics.py`, `routers/metrics.py`, `routers/outcomes.py`, `routers/execution_modes.py`, `routers/exceptions.py` implementations | `routes/operations/*` with compatibility module aliases in `routers/` | Moves operational reporting, alerting, SSE, reliability, outcome/risk profiles, execution-mode discovery, exception workflow/retry policies, dashboard metrics, and Prometheus collectors together under one operations domain. |
