...
- Added `sardis_api.routing.evidence` evidence-domain registrars and moved audit-anchor, evidence capture/export, and attestation wiring out of `sardis_api.main`.
...
- Moved the money movement and ledger route implementations to `sardis_api.routes.money_movement`: ledger, holds, transactions, refunds, payment objects, batch payments, streaming payments, FX, swaps, cross-chain bridge, settlements, and receipts now share the same domain directory with temporary compatibility wrappers under `sardis_api.routers`.
...
- Moved Stripe Connect and Stripe Issuing funding route implementations into `sardis_api.routes.providers`, keeping provider onboarding, callbacks, and funding rail surfaces together.
...
- Moved commerce route implementations into `sardis_api.routes.commerce`: checkout, checkout controls, merchant checkout, merchants, invoices, service directory, counterparties, marketplace, and escrow/dispute APIs now share one contributor-readable domain with compatibility wrappers under `sardis_api.routers`.
- Moved billing, recurring subscription, and metered usage route implementations into `sardis_api.routes.billing` so subscription, checkout, provider, webhook, and usage-reporting APIs share one billing domain.
- Moved operational route implementations into `sardis_api.routes.operations`: analytics, alerts, websocket alerts, event stream, reports, reliability, outcomes, execution modes, exception workflows/retry policies, emergency incident response, dashboard metrics, and Prometheus metrics now share one contributor-readable operations domain.
- Moved remaining mandate route implementations into `sardis_api.routes.authority`: spending mandate CRUD/lifecycle, mandate delegation, and mandate-based subscriptions now live with the core authority and approval APIs.
- Moved delegated credential route implementation into `sardis_api.routes.authority`, keeping credential provisioning, consent validation, scope tightening, and revocation with the rest of the scoped authority APIs.
- Moved enterprise support ticket/profile routes, public SDK install metrics, notification webhook configuration, environment templates, workflow templates, dry-run simulation, dev faucet utilities, and testnet faucet routes into `sardis_api.routes.developer` so contributor-facing support, transparency, webhook setup, testing, and onboarding APIs are no longer buried in the flat router bucket.
- Removed the temporary `sardis_api.routers` compatibility package after migrating internal source and tests to the new domain route modules.
...
- Deleted `packages/api/src/sardis_api/routers/plugins.py`, an unregistered plugin-management API prototype with no live app wiring.
...
Latest API layout pass: the first route naming cleanup removed one dead prototype router and renamed the generic outbound webhook module to make room for a clearer distinction between customer webhook subscriptions and inbound provider callbacks. Route registration extraction has started under `sardis_api.routing.developer`, `sardis_api.routing.money_movement`, and `sardis_api.routing.authority`, so `main.py` can shrink toward domain registrars. The physical route placement pass is now complete: outbound webhook subscription and dev utility route code lives under `sardis_api.routes.developer`, inbound provider callback, provider adapter, Stripe Connect, and Stripe funding route code lives under `sardis_api.routes.providers`, the pay/ledger/transaction/payment/bridge route code lives under `sardis_api.routes.money_movement`, mandate/AP2/MVP/approval/delegation/subscription/credential/facility route code lives under `sardis_api.routes.authority`, wallet/card/treasury/funding/ramp/on-chain/onramp route code lives under `sardis_api.routes.wallets`, billing/subscription/usage route code lives under `sardis_api.routes.billing`, policy definition/simulation/analytics/fallback route code lives under `sardis_api.routes.policy`, evidence/audit/attestation route code lives under `sardis_api.routes.evidence`, compliance/KYC route code lives under `sardis_api.routes.compliance`, x402/MPP/A2A/ACP/ERC-8183/SPT route code lives under `sardis_api.routes.protocol`, FIDES identity/agent-auth/trust route code lives under `sardis_api.routes.identity`, account/auth/organization route code lives under `sardis_api.routes.accounts`, checkout/merchant/invoice/counterparty/marketplace/escrow/secure-checkout route code lives under `sardis_api.routes.commerce`, operational alert/report/event/reliability/exception/emergency/metrics route code lives under `sardis_api.routes.operations`, and agent lifecycle route code lives under `sardis_api.routes.agents`. The old `sardis_api.routers` compatibility package has been removed after internal imports and tests moved to the domain modules.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routers/pay.py packages/api/tests/test_pay_phase3_fx.py` passed after `/api/v2/pay` idempotency changes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routers/batch_payments.py packages/api/tests/test_batch_payments_idempotency.py` passed after batch payment idempotency changes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routers/transactions.py packages/api/tests/test_transactions_batch_idempotency.py` passed after transactions batch idempotency changes.
...
- `python3 -m compileall -q packages/api/scripts/generate_openapi.py packages/api/src/sardis_api/main.py` passed.
...
- `python3 -m compileall -q packages/api/src/sardis_api/main.py packages/api/src/sardis_api/routers/swap.py` passed.
...
- `python3 -m compileall -q packages/api/src/sardis_api` passed after the API route naming cleanup.
...
- `python3 -m compileall -q packages/api/src/sardis_api/main.py packages/api/src/sardis_api/routing packages/api/src/sardis_api/routers/webhook_subscriptions.py` passed after extracting the developer route registrar.
...
- `python3 -m compileall -q packages/api/src/sardis_api/main.py packages/api/src/sardis_api/routing packages/api/src/sardis_api/routers/pay.py` passed after extracting the pay route registrar.
...
- `python3 -m compileall -q packages/api/src/sardis_api/main.py packages/api/src/sardis_api/routing packages/api/src/sardis_api/routers/mandates.py packages/api/src/sardis_api/routers/ap2.py packages/api/src/sardis_api/routers/mvp.py packages/api/src/sardis_api/routers/approvals.py` passed after extracting the authority route registrar.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes packages/api/src/sardis_api/routers/webhook_subscriptions.py packages/api/src/sardis_api/routing/developer.py` passed after the first physical route move.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/authority packages/api/src/sardis_api/routers/ap2.py packages/api/src/sardis_api/routers/approval_config.py packages/api/src/sardis_api/routers/approvals.py packages/api/src/sardis_api/routers/mandates.py packages/api/src/sardis_api/routers/mvp.py packages/api/src/sardis_api/routing/authority.py ...` passed after moving authority route implementations.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/money_movement ... packages/api/src/sardis_api/main.py packages/api/tests/test_transactions_batch_idempotency.py packages/api/tests/test_batch_payments_idempotency.py` passed after moving money movement route implementations.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/wallets ... packages/api/src/sardis_api/main.py ... focused wallet/card tests` passed after moving card, treasury, CPN, and funding capability routes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/wallets/funding.py packages/api/src/sardis_api/routes/wallets/ramp.py packages/api/src/sardis_api/routes/wallets/offramp.py ... packages/api/src/sardis_api/main.py` passed after moving funding/ramp/offramp routes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/wallets/onchain_payments.py packages/api/src/sardis_api/routers/onchain_payments.py packages/api/src/sardis_api/main.py packages/api/tests/test_onchain_payments.py` passed after moving the on-chain payment route.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/wallets/onramp.py packages/api/src/sardis_api/routers/onramp.py packages/api/src/sardis_api/main.py packages/api/tests/test_turnkey_onramp.py packages/api/tests/test_conduit_onramp.py` passed after moving the onramp route.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/wallets/wallets.py packages/api/src/sardis_api/routers/wallets.py packages/api/src/sardis_api/main.py packages/api/tests/test_wallets_x402.py packages/api/tests/test_policy_enforcement.py packages/api/tests/test_spend_recording.py packages/api/tests/test_compliance_gate.py` passed after moving the core wallet route.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/providers packages/api/src/sardis_api/routers/stripe_webhooks.py packages/api/src/sardis_api/routers/stripe_spt_webhooks.py packages/api/src/sardis_api/routers/mastercard_webhooks.py packages/api/src/sardis_api/routers/visa_tap_webhooks.py packages/api/src/sardis_api/routers/partner_card_webhooks.py packages/api/src/sardis_api/routers/cpn_webhooks.py packages/api/src/sardis_api/routers/polar_webhook.py packages/api/src/sardis_api/main.py packages/api/tests/test_partner_card_webhooks.py` passed after moving provider callback routes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/policy packages/api/src/sardis_api/routers/policies.py packages/api/src/sardis_api/routers/policy_simulation.py packages/api/src/sardis_api/routers/policy_analytics.py packages/api/src/sardis_api/routers/fallback_policies.py packages/api/src/sardis_api/main.py packages/api/tests/test_policies_router_dependencies.py packages/api/tests/test_policy_lifecycle.py packages/api/tests/test_policy_simulation.py packages/api/tests/test_fallback_policies_router.py` passed after moving policy routes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/evidence packages/api/src/sardis_api/routers/evidence.py packages/api/src/sardis_api/routers/evidence_export.py packages/api/src/sardis_api/routers/audit_anchors.py packages/api/src/sardis_api/routers/attestation.py packages/api/src/sardis_api/main.py packages/api/tests/test_attestation_endpoint.py packages/api/tests/test_evidence_export.py` passed after moving evidence routes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/compliance packages/api/src/sardis_api/routers/compliance.py packages/api/src/sardis_api/routers/compliance_export.py packages/api/src/sardis_api/routers/kyc_onboarding.py packages/api/src/sardis_api/main.py packages/api/tests/test_compliance_kya_api.py packages/api/tests/test_compliance_audit_api.py packages/api/tests/test_didit_webhook.py packages/api/tests/test_kyc_onboarding.py` passed after moving compliance/KYC routes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/protocol packages/api/src/sardis_api/routers/x402.py packages/api/src/sardis_api/routers/mpp.py packages/api/src/sardis_api/routers/mpp_demo.py packages/api/src/sardis_api/main.py` passed after moving x402/MPP routes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/protocol packages/api/src/sardis_api/routers/a2a.py packages/api/src/sardis_api/routers/a2a_payments.py packages/api/src/sardis_api/routers/acp.py packages/api/src/sardis_api/routers/erc8183.py packages/api/src/sardis_api/routers/spt.py packages/api/src/sardis_api/main.py` passed after moving the remaining protocol-adapter routes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/identity packages/api/src/sardis_api/routers/fides_identity.py packages/api/src/sardis_api/routers/agent_auth.py packages/api/src/sardis_api/routers/trust.py packages/api/src/sardis_api/routes/policy/policies.py packages/api/src/sardis_api/main.py` passed after moving identity/trust routes.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/agents packages/api/src/sardis_api/routers/agent_activity.py packages/api/src/sardis_api/routers/agent_events.py packages/api/src/sardis_api/routers/agent_heartbeat.py packages/api/src/sardis_api/main.py` passed after moving agent lifecycle routes.
...
- The package-path simplification migration renamed `packages/sardis-api` to `packages/api`, removing the most confusing contributor-facing `sardis-api/src/sardis_api` repetition while preserving the Python import package.
...
- `python3 scripts/package_maturity_check.py`, `pnpm check:openapi`, `python3 -m compileall -q packages/api/src/sardis_api packages/api/scripts/generate_openapi.py api/index.py`, and `git diff --check` passed after the package-directory rename.
- `python3 -m compileall -q packages/api/src/sardis_api/routes/accounts packages/api/src/sardis_api/routers/auth.py packages/api/src/sardis_api/routers/email_verification.py packages/api/src/sardis_api/routers/me.py packages/api/src/sardis_api/routers/groups.py packages/api/src/sardis_api/routers/api_keys.py packages/api/src/sardis_api/main.py packages/api/tests/test_signup.py packages/api/tests/test_auth_admin_failopen.py` passed after moving account routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.auth`, `email_verification`, `me`, `groups`, and `api_keys` imports resolve to the same module objects as the new `sardis_api.routes.accounts.*` imports.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/accounts/organizations.py packages/api/src/sardis_api/routers/organizations.py`, an old/new import identity check, `pnpm check:openapi`, `python3 scripts/package_maturity_check.py`, and `git diff --check` passed after moving the unregistered organization route module into the account domain.
- `python3 -m compileall -q packages/api/src/sardis_api/routes/commerce packages/api/src/sardis_api/routers/merchant_checkout.py packages/api/src/sardis_api/routers/merchants.py packages/api/src/sardis_api/routers/checkout.py packages/api/src/sardis_api/routers/checkout_controls.py packages/api/src/sardis_api/routers/invoices.py packages/api/src/sardis_api/main.py` passed after moving commerce routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.merchant_checkout`, `merchants`, `checkout`, `checkout_controls`, and `invoices` imports resolve to the same module objects as the new `sardis_api.routes.commerce.*` imports.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/operations packages/api/src/sardis_api/routers/analytics.py packages/api/src/sardis_api/routers/alerts.py packages/api/src/sardis_api/routers/ws_alerts.py packages/api/src/sardis_api/routers/event_stream.py packages/api/src/sardis_api/routers/reports.py packages/api/src/sardis_api/routers/reliability.py packages/api/src/sardis_api/routers/dashboard_metrics.py packages/api/src/sardis_api/main.py packages/api/src/sardis_api/routes/agents/agent_events.py packages/api/src/sardis_api/routes/agents/agent_heartbeat.py` passed after moving operations routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old operations route imports resolve to the same module objects as the new `sardis_api.routes.operations.*` imports.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/operations/metrics.py packages/api/src/sardis_api/routers/metrics.py packages/api/src/sardis_api/main.py packages/api/src/sardis_api/lifespan.py packages/api/src/sardis_api/middleware/logging.py packages/api/src/sardis_api/middleware/agent_payment_rate_limit.py packages/api/src/sardis_api/services/facility_gate_replay.py packages/api/src/sardis_api/routes/authority/approvals.py packages/api/src/sardis_api/routes/wallets/onchain_payments.py packages/api/src/sardis_api/routers/stripe_funding.py packages/api/src/sardis_api/routers/facility_requests.py packages/api/tests/test_metrics_hardening.py packages/api/tests/test_stripe_funding.py` passed after moving metrics.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.metrics` resolves to the same module object as `sardis_api.routes.operations.metrics`.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/authority/spending_mandates.py packages/api/src/sardis_api/routes/authority/mandate_delegation.py packages/api/src/sardis_api/routes/authority/mandate_subscriptions.py packages/api/src/sardis_api/routers/spending_mandates.py packages/api/src/sardis_api/routers/mandate_delegation.py packages/api/src/sardis_api/routers/mandate_subscriptions.py packages/api/src/sardis_api/main.py` passed after moving the remaining mandate authority routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.spending_mandates`, `mandate_delegation`, and `mandate_subscriptions` imports resolve to the same module objects as the new `sardis_api.routes.authority.*` imports.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/commerce/counterparties.py packages/api/src/sardis_api/routes/commerce/marketplace.py packages/api/src/sardis_api/routes/commerce/escrow_disputes.py packages/api/src/sardis_api/routers/counterparties.py packages/api/src/sardis_api/routers/marketplace.py packages/api/src/sardis_api/routers/escrow_disputes.py packages/api/src/sardis_api/main.py` passed after moving commerce counterparty, marketplace, and escrow/dispute routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.counterparties`, `marketplace`, and `escrow_disputes` imports resolve to the same module objects as the new `sardis_api.routes.commerce.*` imports.
...
- `pnpm check:openapi`, `python3 scripts/package_maturity_check.py`, and `git diff --check` passed after moving commerce routes. OpenAPI remained 540 paths and 592 schemas.
- `python3 -m compileall -q packages/api/src/sardis_api/routes/providers/stripe_connect.py packages/api/src/sardis_api/routes/providers/stripe_funding.py packages/api/src/sardis_api/routers/stripe_connect.py packages/api/src/sardis_api/routers/stripe_funding.py packages/api/src/sardis_api/main.py packages/api/tests/test_stripe_connect.py packages/api/tests/test_stripe_funding.py` passed after moving Stripe provider routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.stripe_connect` and `sardis_api.routers.stripe_funding` imports resolve to the same module objects as the new `sardis_api.routes.providers.*` imports.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/billing/subscriptions.py packages/api/src/sardis_api/routers/subscriptions.py packages/api/src/sardis_api/main.py packages/api/tests/test_recurring_subscriptions.py` passed after moving recurring subscription routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.subscriptions` resolves to the same module object as `sardis_api.routes.billing.subscriptions`.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/authority/credentials.py packages/api/src/sardis_api/routers/credentials.py packages/api/src/sardis_api/main.py packages/api/tests/test_credentials_router.py` passed after moving delegated credential routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.credentials` resolves to the same module object as `sardis_api.routes.authority.credentials`.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/money_movement/bridge.py packages/api/src/sardis_api/routers/bridge.py packages/api/src/sardis_api/main.py` passed after moving cross-chain bridge routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.bridge` resolves to the same module object as `sardis_api.routes.money_movement.bridge`.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/developer/dev.py packages/api/src/sardis_api/routers/dev.py packages/api/src/sardis_api/main.py` passed after moving developer utility routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.dev` resolves to the same module object as `sardis_api.routes.developer.dev`, and confirmed `sardis_api.routers.plugins` is no longer importable.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/operations/exceptions.py packages/api/src/sardis_api/routers/exceptions.py packages/api/src/sardis_api/main.py packages/api/tests/test_exceptions_retry_policies.py` passed after moving exception management routes.
- `PYTHONPATH="$(find packages -maxdepth 2 -type d -name src | tr '\n' ':')" python3 - <<'PY' ...` verified old `sardis_api.routers.exceptions` resolves to the same module object as `sardis_api.routes.operations.exceptions`, preserving the process-local exception engine and retry policy stores used by tests.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routes/operations/emergency.py packages/api/src/sardis_api/routers/emergency.py packages/api/src/sardis_api/main.py` passed after moving emergency incident-response routes.
...
- `rg -n "sardis_api\\.routers|from sardis_api\\.routers|from \\.routers|import sardis_api\\.routers" packages/api/src packages/api/tests` returned no source or test references after removing the compatibility router package.
- `python3 -m compileall -q packages/api/src/sardis_api ... focused route-import tests` passed after removing the compatibility router package.
...
- `python3 -m compileall -q packages/api/src/sardis_api/routing/evidence.py packages/api/src/sardis_api/main.py packages/api/src/sardis_api/routes/evidence packages/api/tests/test_attestation_endpoint.py packages/api/tests/test_evidence_export.py` passed after extracting evidence route registration.
...
- `packages/api/src/sardis_api/main.py` remains an oversized composition root, but OpenAPI generation now has a duplicate-clean check command before router extraction work.
...
- Continue extracting `sardis_api.main` route registration into smaller domain registrars now that route implementation placement is domain-based.
