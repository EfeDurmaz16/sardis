...
- Moved operational route implementations into `sardis_api.routes.operations`: analytics, alerts, websocket alerts, event stream, reports, reliability, outcomes, execution modes, exception workflows/retry policies, dashboard metrics, and Prometheus metrics now share one contributor-readable operations domain.
