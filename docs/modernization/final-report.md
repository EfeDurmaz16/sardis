...
- Moved commerce route implementations into `sardis_api.routes.commerce`: checkout, checkout controls, merchant checkout, merchants, invoices, and service directory now share one contributor-readable domain with compatibility wrappers under `sardis_api.routers`.
