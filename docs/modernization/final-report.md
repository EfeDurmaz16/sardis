...
- Moved account route implementations into `sardis_api.routes.accounts`: auth, email verification, current-account state, groups, and API keys now sit in a contributor-readable account domain with compatibility wrappers under `sardis_api.routers`.
