...
- Moved enterprise support ticket/profile routes, public SDK install metrics, notification webhook configuration, environment templates, and workflow templates into `sardis_api.routes.developer` so contributor-facing support, transparency, webhook setup, and onboarding APIs are no longer buried in the flat router bucket.
