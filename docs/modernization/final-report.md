...
- Moved operational route implementations into `sardis_api.routes.operations`: analytics, alerts, websocket alerts, event stream, reports, reliability, and dashboard metrics now share one contributor-readable operations domain; `metrics.py` remains a separate stateful collector move.
