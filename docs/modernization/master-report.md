...
- `packages/server-api/src/sardis_server/main.py` is too large and centralizes too many unrelated concerns.
...
- Migration systems: `packages/server-api/alembic/versions`, `packages/server-api/migrations`.
...
- Production guards in `packages/server-api/src/sardis_server/lifespan.py`.
...
- `packages/server-api/src/sardis_server/bootstrap/` for middleware, services, routers, and production guards.
- `packages/server-api/src/sardis_server/domains/<domain>/` for cohesive payment, policy, wallet, billing, webhook, evidence, and facility modules over time.
