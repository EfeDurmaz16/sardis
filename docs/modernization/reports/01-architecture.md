...
- Evidence: `packages/server-api/src/sardis_server/main.py` contains middleware setup, provider construction, dependency overrides, router registration, feature flags, checkout setup, billing, facility-gate wiring, and health registration in one `create_app` path. Router registration spans hundreds of lines around `packages/server-api/src/sardis_server/main.py:781`, `packages/server-api/src/sardis_server/main.py:1891`, and `packages/server-api/src/sardis_server/main.py:2244`.
...
- Validation method: `uv run pytest packages/server-api/tests/ -q` plus app startup smoke importing `sardis_server.main:create_app`.

### Medium: Backend still mixes v1, v2, and prototype naming

- Evidence: `packages/server-api/src/sardis_server/main.py` registers both `/api/v1/auth` and `/api/v2/auth`; it also imports `sardis_v2_core` throughout. The public Python client facade now lives at `src/sardis_server/`.
