...
- Evidence: `packages/api/sardis_server/main.py` contains middleware setup, provider construction, dependency overrides, router registration, feature flags, checkout setup, billing, facility-gate wiring, and health registration in one `create_app` path. Router registration spans hundreds of lines around `packages/api/sardis_server/main.py:781`, `packages/api/sardis_server/main.py:1891`, and `packages/api/sardis_server/main.py:2244`.
...
- Evidence: `packages/api/sardis_server/main.py` registers both `/api/v1/auth` and `/api/v2/auth`; it also imports `sardis_v2_core` throughout. The public Python client facade now lives at `sardis_server/`.
