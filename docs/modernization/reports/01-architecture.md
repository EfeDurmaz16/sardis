...
- Evidence: `packages/api/src/sardis_api/main.py` contains middleware setup, provider construction, dependency overrides, router registration, feature flags, checkout setup, billing, facility-gate wiring, and health registration in one `create_app` path. Router registration spans hundreds of lines around `packages/api/src/sardis_api/main.py:781`, `packages/api/src/sardis_api/main.py:1891`, and `packages/api/src/sardis_api/main.py:2244`.
...
- Validation method: `uv run pytest packages/api/tests/ -q` plus app startup smoke importing `sardis_api.main:create_app`.

### Medium: Backend still mixes v1, v2, and prototype naming

- Evidence: `packages/api/src/sardis_api/main.py` registers both `/api/v1/auth` and `/api/v2/auth`; it also imports `sardis_v2_core` throughout. Root package exists at `sardis/`.
