...
- Evidence: root `uv.lock` plus package-local `uv.lock` in `packages/reference-api`, `sardis-chain`, `sardis-cli`, `sardis-connect`, `sardis-core`, `sardis-guardrails`, `sardis-ledger`, `sardis-sdk-python`, and `sardis-striga`.
