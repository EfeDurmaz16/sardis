...
- Evidence: Alembic versions stop at `packages/api/alembic/versions/030_facility_gate.py`, while raw SQL migrations continue to `packages/api/migrations/106_agent_registry.sql`.
...
- Evidence: `packages/api/migrations/049_placeholder.sql` inserts version 49 only to preserve numbering.
...
- Evidence: `packages/api/data/mandates.db` and `packages/api/data/replay_cache.db` are present locally but ignored by `.gitignore`.
