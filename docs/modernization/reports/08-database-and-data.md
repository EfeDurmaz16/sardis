...
- Evidence: Alembic versions stop at `packages/server-api/alembic/versions/030_facility_gate.py`, while raw SQL migrations continue to `packages/server-api/migrations/106_agent_registry.sql`.
...
- Evidence: `packages/server-api/migrations/049_placeholder.sql` inserts version 49 only to preserve numbering.
...
- Evidence: `packages/server-api/data/mandates.db` and `packages/server-api/data/replay_cache.db` are present locally but ignored by `.gitignore`.
