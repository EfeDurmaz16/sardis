...
- Evidence: `apps/api/server/routes/commerce/merchant_checkout.py` logs mandate lookup/validation errors and can continue to payment execution.
...
- Evidence: `apps/api/server/routes/compliance/kyc_onboarding.py` logs invalid raw bodies and stores payload-derived metadata.
...
- Evidence: `apps/api/server/idempotency.py` Redis path compares `request_hash`, but DB fallback records do not consistently read/write and compare that hash.
...
- Evidence: TAP enforcement is enabled by default only in production in `apps/api/server/main.py`; x402 is feature-flagged; production guards live in `apps/api/server/lifespan.py`.
