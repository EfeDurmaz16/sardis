...
- Evidence: `packages/server-api/src/sardis/routers/merchant_checkout.py` logs mandate lookup/validation errors and can continue to payment execution.
...
- Evidence: `packages/server-api/src/sardis/routers/kyc_onboarding.py` logs invalid raw bodies and stores payload-derived metadata.
...
- Evidence: `packages/server-api/src/sardis/idempotency.py` Redis path compares `request_hash`, but DB fallback records do not consistently read/write and compare that hash.
...
- Evidence: TAP enforcement is enabled by default only in production in `packages/server-api/src/sardis/main.py`; x402 is feature-flagged; production guards live in `packages/server-api/src/sardis/lifespan.py`.
