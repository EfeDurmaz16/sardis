...
- Evidence: `packages/api/src/sardis_api/routers/merchant_checkout.py` logs mandate lookup/validation errors and can continue to payment execution.
...
- Evidence: `packages/api/src/sardis_api/routers/kyc_onboarding.py` logs invalid raw bodies and stores payload-derived metadata.
...
- Evidence: `packages/api/src/sardis_api/idempotency.py` Redis path compares `request_hash`, but DB fallback records do not consistently read/write and compare that hash.
...
- Evidence: TAP enforcement is enabled by default only in production in `packages/api/src/sardis_api/main.py`; x402 is feature-flagged; production guards live in `packages/api/src/sardis_api/lifespan.py`.
