...
- Evidence: `packages/sardis-api/src/sardis_api/main.py` constructs many repositories/providers and imports optional modules inside `create_app`.
...
- Evidence: production guards in `packages/sardis-api/src/sardis_api/lifespan.py` reject missing Redis/JWKS in production-like modes, but many repositories still fall back to `memory://` in app setup.
