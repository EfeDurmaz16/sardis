...
- Evidence: `packages/api/src/sardis_server/main.py` constructs many repositories/providers and imports optional modules inside `create_app`.
...
- Evidence: production guards in `packages/api/src/sardis_server/lifespan.py` reject missing Redis/JWKS in production-like modes, but many repositories still fall back to `memory://` in app setup.
