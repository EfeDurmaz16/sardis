...
Root `pyproject.toml` also points pytest at `packages/api/tests` by
default.
...
generated-artifact guard, public doc local-link guard, package maturity matrix,
...
| Test kind | Target |
| --- | --- |
| API route and middleware tests | `packages/api/tests/` |
