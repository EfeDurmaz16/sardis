...
| --- | --- | --- | --- | --- |
| Contributor surface | `.github/workflows/ci.yml` | `Contributor Gate` | OSS/private boundary, stale API paths, generated-artifact hygiene, public doc links, CI inventory, workflow secret scoping, workflow toolchain drift, package maturity, contribution map coverage, root-test backlog drift, and a small API/bootstrap smoke suite. | `pnpm run check:contributor` |
