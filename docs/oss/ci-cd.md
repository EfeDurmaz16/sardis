...
| Python API and core packages | `.github/workflows/ci.yml` | `Python Lint & Test` | Ruff, maintained API tests, package-owned core/ledger/chain suites, and OpenAPI route snapshot stability. | `pnpm run check:openapi`; `uv run pytest packages/reference-api/tests/ -q` |
| API-focused integration gate | `.github/workflows/test-api.yml` | `API Lint & Test` | API/core lint, package API tests against test Postgres, and Facility Gate tabletop/readiness artifacts. | `uv run pytest packages/reference-api/tests/ -q` |
