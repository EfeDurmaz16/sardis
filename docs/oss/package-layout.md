...
| route files named after their parent folder | Names like `routes/billing/billing.py`, `routes/compliance/compliance.py`, and old `routes/agents/agents.py` make navigation feel duplicated even when the domain folder is correct. | Rename one route module at a time to describe the route role, for example lifecycle, accounts, screening, or webhooks. Preserve public HTTP paths and route registration behavior. |
