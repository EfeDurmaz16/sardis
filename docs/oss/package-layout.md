...
| `packages/sardis-core/src/sardis_v2_core/` | The `v2` import name looks stale and prototype-like even though the package is core. | Plan a compatibility-preserving import migration. Add `sardis_core` as the preferred import, keep `sardis_v2_core` as a deprecated shim, and update internal imports in atomic commits. |
