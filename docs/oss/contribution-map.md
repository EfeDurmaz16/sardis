...
| Agent ecosystem experiments | `packages/sardis-adk/`, `packages/sardis-agentkit/`, `packages/sardis-browser-use/`, `packages/sardis-composio/`, `packages/sardis-autogpt/`, `packages/sardis-openai/`, `packages/sardis-openclaw/`, `packages/sardis-gpt/`, `packages/sardis-e2b/` | Refresh against current upstream APIs, add minimal smoke tests, archive stale packages when justified | package README command or a new package-owned smoke test |
