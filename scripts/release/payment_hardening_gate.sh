...
require_file "docs/security/payment-hardening/preprod-gate.md"
require_file "docs/security/payment-hardening/slo-alerts.md"
require_file "docs/security/payment-hardening/pci-approvals-and-db-hardening-checklist.md"
...
require_match 'policy_pin_requires_active_policy' \
  'packages/reference-api/sardis_server/routes/wallets/onchain_payments.py' \
...
require_match 'cursor_scope_mismatch' \
  'packages/reference-api/sardis_server/routes/compliance/compliance.py' \
...
require_match 'card_details_invalid' \
  'packages/reference-api/sardis_server/routes/commerce/secure_checkout.py' \
...
require_match '_check_a2a_trust_relation' \
  'packages/reference-api/sardis_server/routes/protocol/a2a.py' \
...
require_match '/asa/security-policy' \
  'packages/reference-api/sardis_server/routes/wallets/cards.py' \
...
require_match 'test_onchain_payment_adversarial_prompt_patterns_require_approval' \
  'packages/reference-api/tests/test_onchain_payments.py' \
...
require_match 'test_topup_all_providers_failed_returns_502_and_records_attempts' \
  'packages/reference-api/tests/test_stripe_funding.py' \
...
require_match 'test_pan_entry_execute_fail_closed_on_invalid_revealed_card_details' \
  'packages/reference-api/tests/test_secure_checkout_executor.py' \
...
require_match 'test_security_incident_emits_severity_and_ops_approval_pending' \
  'packages/reference-api/tests/test_secure_checkout_risk_response.py' \
...
require_match 'test_a2a_trust_table_rejects_untrusted_pair' \
  'packages/reference-api/tests/test_a2a_trust_table.py' \
  'A2A trust table rejection test must exist'
require_match 'test_ensure_table_prod_requires_migration' \
  'packages/reference-api/tests/test_a2a_trust_repository.py' \
...
require_match 'test_trust_peers_returns_only_trusted_by_default' \
  'packages/reference-api/tests/test_a2a_trust_endpoints.py' \
...
require_match 'test_cards_asa_security_policy_endpoint' \
  'packages/reference-api/tests/test_cards_provider_introspection.py' \
...
if [[ "${RUN_PAYMENT_HARDENING_TESTS:-0}" == "1" ]]; then
  if ! command -v pytest >/dev/null 2>&1; then
    ...
    if ! pytest -q \
      packages/reference-api/tests/test_onchain_payments.py \
      packages/reference-api/tests/test_compliance_audit_api.py \
      packages/reference-api/tests/test_secure_checkout_executor.py \
      packages/reference-api/tests/test_secure_checkout_risk_response.py \
      packages/reference-api/tests/test_a2a_trust_table.py \
      packages/reference-api/tests/test_a2a_trust_endpoints.py \
