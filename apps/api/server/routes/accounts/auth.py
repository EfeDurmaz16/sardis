...
async def bootstrap_api_key(
    ...
    from server.middleware.auth import get_api_key_manager
...
async def signup(request: Request, body: SignupRequest):
    ...
    try:
        from server.email_templates import send_welcome_email
...
async def register_user(request: Request, body: RegisterRequest):
    ...
    from server.analytics.posthog_tracker import SIGNUP_COMPLETED, identify_user, track_event
...
async def forgot_password(request: Request, body: ForgotPasswordRequest):
    ...
    if token:
        from server.email_templates import send_password_reset_email
...
async def delete_account(request: Request, user: UserInfo = Depends(require_auth)):
    ...
    from server.analytics.posthog_tracker import track_event
...
async def _resolve_principal(
    ...
    from server.middleware.auth import get_api_key
