...
from server.routes.operations.ws_alerts import get_connection_manager
        ...
        if pagerduty_routing_key:
            _dispatcher.register_channel(
                ...
                PagerDutyChannel(
                    ...
                    source=os.getenv("PAGERDUTY_SOURCE", "server.api"),
                ),
            )
    ...
    if channel_type == "slack":
        ...
        deps.dispatcher.register_channel(
            ...
            PagerDutyChannel(
                ...
                source=request.config.get("source", "server.api"),
            ),
            enabled=request.enabled,
        )
