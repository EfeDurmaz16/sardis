...
const CHAIN_COLORS: Record<string, string> = { base: '#0052FF', base_sepolia: '#0052FF', polygon: '#8247E5', arbitrum: '#28A0F0', optimism: '#FF0420', ethereum: '#627EEA' }

function ChainBadge({ chain }: { chain?: string }) { if (!chain) return null; const color = CHAIN_COLORS[chain] || '#808080'; const label = chain.replace('_sepolia', ' (testnet)'); return <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium capitalize" style={{ background: `${color}20`, color, border: `1px solid ${color}30` }}><span className="w-1.5 h-1.5 rounded-full" style={{ background: color }} />{label}</span> }
function CopyButton({ text, size = 14 }: { text: string; size?: number }) { const [copied, setCopied] = useState(false); const handleCopy = () => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 1500) }; return <button onClick={handleCopy} className="p-1 text-gray-500 hover:text-white transition-colors" title="Copy">{copied ? <Check size={size} className="text-green-400" /> : <Copy size={size} />}</button> }
function truncateHash(hash: string, chars = 6): string { if (hash.length <= chars * 2 + 2) return hash; return `${hash.slice(0, chars + 2)}...${hash.slice(-chars)}` }

interface Transaction { tx_id: string; from_wallet: string; to_wallet: string; amount: string; fee: string; currency: string; purpose: string; status: 'completed' | 'pending' | 'failed'; created_at: string; chain?: string; chain_tx_hash?: string; tx_hash?: string; block_number?: number; gas_used?: string }
type StatusFilter = 'all' | 'completed' | 'pending' | 'failed'

export default function TransactionsPage() {
  const [search, setSearch] = useState(''); const [statusFilter, setStatusFilter] = useState<StatusFilter>('all'); const [transactions, setTransactions] = useState<Transaction[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null); const [expandedTxId, setExpandedTxId] = useState<string | null>(null);
  const fetchTransactions = useCallback(async () => { try { setLoading(true); setError(null); const data = await ledgerApi.recent(100); setTransactions(data as Transaction[]) } catch (err) { setError(err instanceof Error ? err.message : 'Failed to load transactions'); setTransactions([]) } finally { setLoading(false) } }, [])
  useEffect(() => { fetchTransactions() }, [fetchTransactions])
  const filteredTransactions = transactions.filter(tx => { const matchesSearch = tx.tx_id.toLowerCase().includes(search.toLowerCase()) || tx.purpose?.toLowerCase().includes(search.toLowerCase()) || (tx.chain_tx_hash || '').toLowerCase().includes(search.toLowerCase()); const matchesStatus = statusFilter === 'all' || tx.status === statusFilter; return matchesSearch && matchesStatus })
  const totalVolume = transactions.filter(tx => tx.status === 'completed').reduce((sum, tx) => sum + parseFloat(tx.amount), 0)
  const toggleExpand = (txId: string) => { setExpandedTxId(prev => prev === txId ? null : txId) }

  return (
    <div className="space-y-8">
      <div><h1 className="text-3xl font-bold text-white font-display">Transactions</h1><p className="text-gray-400 mt-1">View all payment activity with on-chain details</p></div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="card p-4"><p className="text-sm text-gray-400">Total Volume</p><p className="text-2xl font-bold text-white mono-numbers">${totalVolume.toFixed(2)}</p></div>
        <div className="card p-4"><p className="text-sm text-gray-400">Transactions</p><p className="text-2xl font-bold text-white">{transactions.length}</p></div>
        <div className="card p-4"><p className="text-sm text-gray-400">Completed</p><p className="text-2xl font-bold text-green-500">{transactions.filter(t => t.status === 'completed').length}</p></div>
        <div className="card p-4"><p className="text-sm text-gray-400">Failed</p><p className="text-2xl font-bold text-red-500">{transactions.filter(t => t.status === 'failed').length}</p></div>
      </div>
          ...
          <tbody className="divide-y divide-dark-100">
            {filteredTransactions.map((tx) => { const txHash = tx.chain_tx_hash || tx.tx_hash; const explorerUrl = getExplorerTxUrl(tx.chain, txHash); const isExpanded = expandedTxId === tx.tx_id; return (
              <> <tr key={tx.tx_id} className={clsx('hover:bg-dark-200/50 transition-colors cursor-pointer', isExpanded && 'bg-dark-200/30')} onClick={() => toggleExpand(tx.tx_id)}>
                ...
                <td className="px-4 py-4"><p className="text-sm font-medium text-white mono-numbers">${tx.amount} {tx.currency}</p>{tx.fee && tx.fee !== '0' && <p className="text-xs text-gray-500">Fee: ${tx.fee}</p>}</td>
                <td className="px-4 py-4"><p className="text-sm text-gray-300 max-w-xs truncate">{tx.purpose || '-'}</p></td>
                <td className="px-4 py-4"><span className={clsx('inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium', tx.status === 'completed' && 'bg-green-500/10 text-green-500', tx.status === 'pending' && 'bg-yellow-500/10 text-yellow-500', tx.status === 'failed' && 'bg-red-500/10 text-red-500')}><div className={clsx('w-1.5 h-1.5 rounded-full', tx.status === 'completed' && 'bg-green-500', tx.status === 'pending' && 'bg-yellow-500 animate-pulse', tx.status === 'failed' && 'bg-red-500')} />{tx.status}</span></td>
