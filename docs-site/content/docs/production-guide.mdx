...
1. Run the Sardis reference API locally or use your team's Sardis API deployment.
2. Issue a sandbox key with `write` scope through the deployment's configured auth provider.
3. Copy the key (starts with `sk_test_`).
4. Store it in your local environment, not in source control.
